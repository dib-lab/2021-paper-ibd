...
While we present a pipeline that enables differential abundance analysis directly on assembly graphs, we identified areas of future that could improve our approach.
First, implementing approaches to better control graph piece size would be beneficial.
We heuristically selected *r* = 10 to build the metapangeome graphs because this radius size produced dominating set pieces with approximately the same number of k-mers as the average bacterial gene for pieces that were present in many metagenomes.
However, the radius size needed to meet this condition may change depending on the diversity of the sequencing reads used to build the metapangenome graph.
...
Algorithms that either automatically select a radius size that achieves a user-specified average piece size, or that produce more consistent piece sizes independent of diversity of the sequencing data would provide finer control of the graph structure and subsequent sequence comparisons.
Switching from a cDBG to a de Bruijn graph as the base spacegraphcats graph structure could lead to more consistent piece sizes; cDBGs have variable node sizes because they combine nodes without branching paths, while every node in a de Bruijn graph is length *k*. 
...
Similarly, it may allow dominating set differential abundance analysis to be performed directly on groups of metagenomes without the need to first identify species of interest via genome queries.